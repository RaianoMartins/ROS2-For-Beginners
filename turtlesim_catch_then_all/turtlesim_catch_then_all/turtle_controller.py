...
class TurtleControllerNode(Node):
    ...
    def call_catch_turtle_server(self, turtle_name):
        ...
        while not client.wait_for_service(1.0):
            self.get_logger().warn("Esperando o servidor...")
    ...
    def callback_call_catch_turtle(self, future, turtle_name):
        try:
            ...
            if not response.success:
                self.get_logger().error("Turtle " + str(turtle_name) + " não pode ser capturada!")
        except Exception as e:
            self.get_logger().error("Chamada de serviço falhou! %r" % (e,))
